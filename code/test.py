...
def main():
    ...
    parser.add_argument(
        '-opt', type=str, required=False, help='Path to options JSON file.', default="code/options/test/test_MAIC_CelebA.json")
    ...
    for bm, test_loader in zip(bm_names, test_loaders):
        ...
        for iter, batch in tqdm(enumerate(test_loader), total=len(test_loader)):
            solver.feed_data(batch, need_HR=need_HR, need_landmark=True)
            ...
            if need_HR:
                ...
                #        (t1 - t0)))
                res_dict[path_list[-1]] = {'psnr': psnr, 'ssim': ssim,'nrnse':nrmse, 'time': t1 - t0}
        ...
        if need_HR:
            ...
            average_res_str = "PSNR: %.2f      SSIM: %.4f  NRMSE: %.4f    Speed: %.4f" % \
                  (sum(total_psnr) / len(total_psnr), sum(total_ssim) /
                   len(total_ssim),sum(total_nrmse)/len(total_nrmse) ,sum(total_time) / len(total_time))
