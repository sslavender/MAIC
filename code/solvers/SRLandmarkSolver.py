...
class SRLandmarkSolver(BaseSolver):
    ...
    def test(self):
        self.model.eval()
        def calculate_nrmse_heatmap(heatmap_pred, heatmap_gt):
        # Assuming heatmap_pred and heatmap_gt are 2D tensors
        # Reshape to 1D tensors
            pred_flat = heatmap_pred.view(-1)
            gt_flat = heatmap_gt.view(-1)

            # Calculate RMSE
            rmse = torch.sqrt(torch.mean((pred_flat - gt_flat)**2))

            # Calculate the range of the ground truth heatmap (max - min)
            gt_range = torch.max(heatmap_gt) - torch.min(heatmap_gt)
        ...
        with torch.no_grad():
            ...
            # SR_list = forward_func(self.LR)
            self.SR_list = SR_list
            self.heatmap_list = heatmap_list
            self.SR = SR_list[-1]
            self.heatmap = heatmap_list[-1]
            loss = calculate_nrmse_heatmap(self.heatmap,self.gt_heatmap)
    ...
    def save_checkpoint(self, epoch, is_best):
        ...
        if is_best:
            self.checkpoint_dir = '/root/code/Deep-Iterative-Collaboration-master/experiments'
            filename = os.path.join(self.checkpoint_dir, 'best_no_ckp.pth')
            print('===> Saving best checkpoint (step %d) to [%s] ...]' % (self.step, filename))
        # else:
        #     self.checkpoint_dir = '/home/dell/code/srface/Deep-Iterative-Collaboration-master/experiments'
    ...
    def get_current_visual(self, need_np=True, need_HR=True):
        ...
        def _get_data(x):
            ...
            if x.dim() == 3:
             return self.unnorm(x).data.float().cpu()
